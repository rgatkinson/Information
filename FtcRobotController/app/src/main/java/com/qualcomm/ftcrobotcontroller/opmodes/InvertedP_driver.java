...
public class InvertedP_driver extends OpMode implements SensorEventListener {
   private static final float F_INITIAL_ANGLE = 0.0F;
   private static final float GYRO_THRESHOLD = 0.1F;
   private static final float deadZone = 2.5F;
   private static final float kA = 0.0F;
   private static final float kD = 0.001F;
   private static final float kDA = 0.0F;
   ...
   private float driverAngle = 0.0F;
   float fAngle = 0.0F;
   ...
   private void adjustAngle() {
      this.currentAngle += 0.0F * (float)this.iDeltaMotorAngle;
      this.currentAngle += 0.0F * (float)this.iMotorAngle;
      double var10000 = (double)(-this.gamepad1.left_stick_y);
   }

   private void adjustMotors() {
      this.fPowerLeft = 1.0F * (this.P + this.I + this.D);
      this.fPowerRight = this.fPowerLeft;
      if((double)this.gamepad1.left_stick_x < -0.1D) {
         this.fPowerLeft -= 0.15F;
         this.fPowerRight += 0.15F;
      } else if((double)this.gamepad1.left_stick_x > 0.1D) {
         this.fPowerLeft += 0.15F;
         this.fPowerRight -= 0.15F;
      }

      if((double)this.gamepad1.left_stick_y > -0.1D) {
         this.fPowerLeft += 0.25F;
         this.fPowerRight += 0.25F;
      } else if((double)this.gamepad1.left_stick_y > 0.1D) {
         this.fPowerLeft -= 0.25F;
         this.fPowerRight -= 0.25F;
      }
   ...
   private void calculatePID() {
      this.currentError = this.currentAngle - (this.targetAngle + this.driverAngle);
      this.currentErrorRate = this.currentAngleRate;
      this.P = 0.003F * this.currentError;
      this.I += 0.0F * this.currentError * (float)this.lDeltaTime;
      this.D = 0.001F * this.currentErrorRate;
   }
   ...
   private void processGyro(SensorEvent var1) {
      ...
      if(Math.abs(this.currentAngleRate) < 0.1F) {

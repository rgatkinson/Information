...
public class InvertedP extends OpMode implements SensorEventListener {
   ...
   float P = 0.0F;
   Context context;
   private float controlVar = 0.0F;
   private float deltaErrorVar = 0.0F;
   private float errorVar = 0.0F;
   float fAngle = 0.0F;
   ...
   private float inputVar = 0.0F;
   long lDeltaTime;
   long lPrevTime;
   DcMotor motorLeft;
   DcMotor motorRight;
   private float prevErrorVar = 0.0F;
   private float processRate = 0.0F;
   private float processVar = 0.0F;
   ...
   private void calculatePID() {
      this.errorVar = this.processVar - this.setPoint;
      this.P = 0.5F * this.errorVar;
      this.fError = this.fAngle;
      this.I += 0.0F * this.fError * (float)this.lDeltaTime;
      this.D = 0.0F * this.fAngleRate / (float)this.lDeltaTime;
   }
   ...
   private void processGyro(SensorEvent var1) {
      this.processRate = var1.values[0];
      long var2 = System.currentTimeMillis();
      this.lDeltaTime = var2 - this.lPrevTime;
      this.lPrevTime = var2;
      this.processVar += this.processRate * (float)this.lDeltaTime;
   }
   ...
   public void init() {
      ...
      this.sensorGyro = this.sensorManager.getDefaultSensor(16);
